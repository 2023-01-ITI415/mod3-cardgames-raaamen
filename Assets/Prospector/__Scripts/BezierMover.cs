...
public class BezierMover : MonoBehaviour
{
    ...
    public void Init(List<Vector2> ptsV2, float timeD = 1, float timeS = 0){
        ...
        foreach(var v2 in ptsV2) {ptsV3.Add((Vector3)v2);}
        Init(ptsV3, timeD, timeS);
    }

    public Vector3 pos{
        get{
            if(posMode == ePosMode.ugui) return rectTrans.anchorMin;
            else if (posMode==ePosMode.local) return transform.localPosition;
            else return transform.position;
        }
        private set{
            if(posMode == ePosMode.ugui) rectTrans.anchorMin = rectTrans.anchorMax=value;
            else if (posMode==ePosMode.local) transform.localPosition = value;
